...
**1. Technical documentation** (~1,300 documents) — product specs, hardware design guides, reference designs, protocol documentation, certificates, application notes. This is the canonical source of truth for "what does the product do and how do you use it."

**2. Support ticket history** (~28 monthly collections spanning 27 months) — past customer issues with root cause analysis and solutions extracted and summarized by AI. This is the institutional memory that prevents reinventing the wheel.

**3. Schematic review cases** (~21 cases across 11 product lines) — past hardware design reviews with risk assessments and recommendations. This trains the AI to conduct new reviews following established patterns.

Separating these into distinct knowledge bases wasn't arbitrary. Each serves a different retrieval pattern:

- Technical docs answer "what is X?" questions
- Support tickets answer "we saw problem Y, what was the fix?" questions
- Schematic cases answer "is this hardware design correct?" questions

**Mixing them into one giant knowledge base would have destroyed retrieval quality.** The vector similarity between "LC module power consumption" in a datasheet and "LC module power issue" in a support ticket is high, but the user intent behind those queries is completely different.
